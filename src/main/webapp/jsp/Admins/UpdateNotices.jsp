...
<body>
	<c:if test="${state == '2'}">
		...
		<form class="form1" action="/Net/jsp/Admins/${messages}" method="post">
			<label for="Nid">通知编号</label><input type="text" id="Nid" name="Nid"
				value="${msg.nid}" /> <label for="Oid">订单编号</label><input
				type="text" id="Oid" name="Oid" value="${msg.oid}" /> <label
				for="Uid">用户编号</label><input type="text" id="Uid" name="Uid"
				value="${msg.uid}" /> <label for="Nstate">通知状态</label><input
				type="text" id="Nstate" name="Nstate" value="${msg.nstate}" /> <input
