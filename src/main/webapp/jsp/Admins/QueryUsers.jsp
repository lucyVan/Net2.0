						...
						<tbody>
							<c:forEach items="${msg}" var="obj">
								<tr>
									...
									<td>${obj.uemail}</td>
									<td>${obj.upassword}</td>
									<td><fmt:formatDate value="${obj.ucreatetime}"
											pattern="yyyy-MM-dd" /></td>
									<td>${obj.uimage}</td>
									<td>${obj.species.sname}</td>
									<td><a style="width: 100%" type="button"
										href="/Net/jsp/Admins/DeleteUsers.do/${obj.uid}"
										class="layui-btn layui-btn-radius  layui-btn-normal">删除</a></td>
									<td><a style="width: 100%" type="button"
										href="/Net/jsp/Admins/UpdateUsers.do/${obj.uid}"
										class="layui-btn layui-btn-radius  layui-btn-normal">修改</a></td>
								</tr>
							</c:forEach>
							<tr>
								<th colspan="11"><a style="width: 100%" type="button"
									href="/Net/jsp/Admins/InsertUsers.do"
							...
							<tr>
								<th colspan="11">
									...
									href="/Net/jsp/Admins/QueryUsers.do?pn=1">首页</a> <c:if
										test="${pageInfo.hasPreviousPage }">
										<a
											href="/Net/jsp/Admins/QueryUsers.do?pn=${pageInfo.pageNum-1}">
											<span>&laquo;</span>
										</a>
										...
										<c:if test="${page_Num != pageInfo.pageNum }">
											<a href="/Net/jsp/Admins/QueryUsers.do?pn=${page_Num }">${page_Num }</a>
										</c:if>
									...
									</c:if> <a href="/Net/jsp/Admins/QueryUsers.do?pn=${pageInfo.pages}">末页</a>
								</th>
							</tr>
						</tbody>
					</table>
