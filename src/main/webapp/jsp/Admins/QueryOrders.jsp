						...
						<tbody>
							<c:forEach items="${msg}" var="obj">
								<tr>
									...
									<td>${obj.oevaul}</td>
									<td>${obj.cprice}</td>
									<td>${obj.ocreatetime}</td>
									<td>${obj.ostate}</td>
									<td><a style="width: 100%" type="button"
										href="/Net/jsp/Admins/DeleteOrders.do/${obj.oid}"
										class="layui-btn layui-btn-radius  layui-btn-normal">删除</a></td>
									<td><a style="width: 100%" type="button"
										href="/Net/jsp/Admins/UpdateOrders.do/${obj.oid}"
										class="layui-btn layui-btn-radius  layui-btn-normal">修改</a></td>
								</tr>
							</c:forEach>
							<tr>
								<th colspan="9"><a style="width: 100%" type="button"
									href="/Net/jsp/Admins/InsertOrders.do"
							...
							<tr>
								<th colspan="9">
									...
									href="/Net/jsp/Admins/QueryOrders.do?pn=1">首页</a> <c:if
										test="${pageInfo.hasPreviousPage }">
										<a
											href="/Net/jsp/Admins/QueryOrders.do?pn=${pageInfo.pageNum-1}">
											<span>&laquo;</span>
										</a>
										...
										<c:if test="${page_Num != pageInfo.pageNum }">
											<a href="/Net/jsp/Admins/QueryOrders.do?pn=${page_Num }">${page_Num }</a>
										</c:if>
									...
									</c:if> <a href="/Net/jsp/Admins/QueryOrders.do?pn=${pageInfo.pages}">末页</a>
								</th>
							</tr>
						</tbody>
					</table>
